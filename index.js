...
async function login() {
  ...
  try {
    ...
    if (error.name === "IgCheckpointError") {
      console.log("Checkpoint error encountered. Handling checkpoint...");
      await ig.challenge.auto(true); // Requesting challenge automatically (SMS or email)
      console.log(ig.state.checkpoint); // Challenge info here

      // Prompt user for the code received via SMS or email
      const { code } = await inquirer.prompt([
        {
          type: "input",
          name: "code",
          message: "Enter the code you received via SMS or Email:",
        },
      ]);

      const result = await ig.challenge.sendSecurityCode(code);
      console.log("Challenge result:", result);

      if (result.status === "ok") {
        console.log("Challenge completed successfully. Logging in again...");
